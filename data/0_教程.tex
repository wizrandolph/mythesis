...
该研究旨在有效微博谣言检测，主要关注文本和用户两个角度，通过分析提取深层次情感及用户权威度等特征、融合用户内容浅层及深层特征，进行分类谣言微博和普通微博的特征来实现谣言检测。研究主要提出MRTU和MRFUF，内容如下：

（1）基于文本特征和用户特征的谣言检测模型 (MRTU)

由于谣言数据集的文本缺乏语义信息和严格的句法结构，结合用户特征和内容特征来丰富语义信息显得很有意义。MRTU 模型在前人研究的基础上，从文本特征和用户特征两方面深入分析特征，重点考虑文本情感特征分析与隐式用户特征的分析。针对文本情感分析，结合新浪微博平台的特点构建语料库以提高情感分析的准确性；针对隐式用户特征，抽取出用户信息完整度、用户活跃度、用户交际广度和用户平台认证指数四项指标构建用户权威度定量计算模型，并将用户权威度及其四项构成指标作为隐式用户特征。同时，使用BERT预训练模型提取文本语义特征，并结合多模态适应门（Multimodal Adaptation Gate，MAG）融合文本特征与用户特征。在微博数据集上进行的实验表明，相比基线模型，MRTU模型的检测性能更优，准确率达0.941。

（2）基于融合转发用户特征的谣言检测模型 (MRFUF）

考虑到转发用户在谣言传播中的作用，本章在第三章模型的基础上，融合转发用户特征，提出了一个融合转发用户特征的微博谣言检测模型（Microblog Rumor detection model based on fusion of Forwarding User Feature，MRFUF )，以期达到提高谣言检测准确率。MRFUF模型提出了两种转发用户特征，一是考虑大多谣言传播过程中出现社交网络水军，借助网络水军数量作为转发用户特征之一；而是考虑到辟谣用户的出现对于判定谣言也重要，将辟谣用户数量作为转发用户特征之一。在微博数据集结合网络水军数据集上的一系列实验表明MRFUF模型的有效性。


\section{研究不足与展望}
\label{sec:font}
本文从内容特征和用户特征两个角度分析谣言微博的特点，提出的MRTU模型和MRFUF模型在微博谣言检测任务上取得了较好的检测效果，但是仍然存在一些不足之处。现有的谣言数据集虽然在帖子总量上达到了较大规模，但在谣言事件层面的数据量上仍有待提高。源帖子、回复帖子、用户信息等多个组成部分共同构成了一个完整的谣言传播过程。然而包含以上所有信息的数据集数量较少，不利于更加全面的探索提升谣言检测效果的方法。

下述是对未来谣言检测任务工作的展望。

（1）构建一个规模更大特征更丰富的数据集。数据集的质量从根本上影响了对各模型检测效果的准确评估。可以加入更多涉及不同主题的相关数据，扩充数据集规模，降低模型过拟合的可能性，以便将 BERT 等更深的预训练模型引入到谣言检测任务中。构建特征更加丰富的数据集有利于模型挖掘潜在特征，加深对谣言传播过程的理解，进一步提升检测效果。

（2）探索外部知识对谣言检测任务的作用。谣言和谣言检测任务的定义随着时代背景的不同而不断变化，单独针对某一个数据集设计的检测模型难以涵盖全部的应用场景。常规语言在社交媒体上的沟通和表述方式逐渐发生了变化，组成帖子的短短数个词语往往包含了极为丰富的特征信息，并可能隐含着特殊的背景信息。这些重要的信息难以直接从帖子的内容层面获取到。为了贴近对帖子真实语义的理解，需要考虑到帖子内部各实体之间知识层次的关系。对这些知识的判断和联系有助于评估帖子的可信度。

（3）未来应引入多任务学习策略。在谣言检测任务的工作流程中，可能涉及到多个不同的子任务，包括用户可信度评估、源帖子可信度评估和回复帖子立场等。对多个子任务进行联合学习可以提取到不同任务间的共有特征，有助于提高谣言检测任务的准确性。
...
% 如果想在表格中使用脚注，minipage是个不错的办法
  ...
  \label{tab:template-files}
    \begin{tabular*}{\linewidth}{lp{10cm}}
      ...
      {\hei 步骤} & {\hei 内容} \\
      \hline
      scnuthesis.ins & \LaTeX{} 安装文件，docstri \\
      scnuthesis.dtx & 所有的一切都在这里面。\\
...
强烈推荐《\LaTeXe 插图指南》！关于子图形使用细节请参看\textsf{subfig}手册。~\ref{fig:ipe}。
...
\begin{figure}[htbp] % use float package if you want it here
  \centering
  \includegraphics[width=\textwidth]{华师硕士毕业论文/figures/图1-1裁剪.pdf}
  \caption{利用TikZ制图}
  \label{fig:ipe}
\end{figure}
